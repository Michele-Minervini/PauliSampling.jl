...
| `init` (+ `init_clamp`, `init_alpha`) | parameter initialization. `:randn` (**default**, warm/safe at all sizes, but high-variance at ≥4×4) · `:data` (full mean-field: fields←pixel marginals, ZZ←pixel correlations; best at 3×3/4×4 but COLD → operator blow-up at ≥5×5) · `:data_warm` (cluster-safe: fields **clamped** `init_clamp` & **scaled** `init_alpha` + data couplings — lower clamp/α = warmer/safer) · `:data_coupling` (random fields + data couplings: warm & stable, modest gain over `:randn`) |
